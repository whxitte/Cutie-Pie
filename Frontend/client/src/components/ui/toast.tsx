...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-right-full",
  {
    variants: {
      variant: {
        default: "border border-[#00ff41]/50 bg-[#1a1a1a] text-white",
        destructive:
          "destructive group border-[#ff3e3e]/50 bg-[#1a1a1a] text-[#ff3e3e]",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-white opacity-70 transition-opacity hover:text-[#00ff41] hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-1 focus:ring-[#00ff41] group-hover:opacity-100 group-[.destructive]:text-white group-[.destructive]:hover:text-[#ff3e3e]",
      className
    )}
    toast-close=""
    {...props}
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn("text-sm font-semibold text-[#00ff41]", className)}
    {...props}
  />
))
